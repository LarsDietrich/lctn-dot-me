...
/**
 * Does the API call to get the webcam data with a callback to processWebcamResults.
 * 
 * @param selectedLocation - the location to check for of type google.maps.LatLng()
 * @param range - the range to check for, in kilometers
 */
...
/**
 * Updates the "Webcam" information, if id is supplied, will call method to load individual webcam in window.
 * 
 * @param id - the position in listOfWebcams array of the webcam being displayed.
 */
...
/**
 * Given a JSON object, will populate listOfWebcams with rows to display. If a number is supplied, will use it to retrieve
 * the relevant item from listOfWebcams and display it in "fullscreen" mode.
 * 
 * @param data -either a JSON object from the API or a number representing the position of the webcam in listOfWebcams array
 */
	...
	if (data.webcams) {
		...
		if (webcams) {
			...
			$.each(webcams, function(index, webcam) {
				...
				var output = "<tr onmouseover='highlightRow(this," + webcam.latitude + "," + webcam.longitude + ", \"" + camera_image + "\", \"/images/webcam-shadow.png\")' onmouseout='normalRow(this)'>";
		...
		if (currentWebcams[data].timelapse && (currentWebcams[data].timelapse.available == 1)) {
			output += "<a href=\"" + currentWebcams[data].timelapse.format_flv + "\" style=\"display:block;width:450px;height:320px;z-index:" + $("#webcam_container").css("z-index") + "\" id=\"player\"></a>";
		} else {
			output += "<img id=\"webcam-full\" title=\"Click for fullscreen\" onclick=\"fullscreenImage('http://images.webcams.travel/webcam/" + currentWebcams[data].webcamid + ".jpg\')\" class=\"webcam-full-image pointer\" src=\"http://images.webcams.travel/webcam/" + currentWebcams[data].webcamid + ".jpg\"/><br/>";
		}
