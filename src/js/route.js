...
function updateRouteDisplay(start, end) {
	...
	if (listOfRoute.length == 0) {
		output += "<tr><td>I have no idea how to get from <b><div class=\"inline\" id=\"route_text_from\">" + start
				+ "</div></b> to <b><div class=\"inline\" id=\"route_text_to\">" + end + "</div></b>, try a different location.</td</tr>";
	} else {
		output += "<tr><td>To get from <b><div class=\"inline\" id=\"route_text_from\">" + start + "</div></b> to <b><div class=\"inline\" id=\"route_text_to\">" + end
...
function getRouteToLocation(location) {
	...
	$("#route_to").val(location);
	$("#route_from").val(selectedLocation.lat() + "," + selectedLocation.lng());
...
function renderAddress(control) {
	var split = $("#route_" + control).val().split(",");
	...
	geocoder.geocode( {
		...
		if (status == google.maps.GeocoderStatus.OK) {
			if (results.length > 0) {
				...
				$("#route_" + control).val(address);
				$("#route_text_" + control).html(address);
			}
		}
	});
}
...
/**
 * Remove the rendered path from the map.
 */
function clearDirectonsFromMap() {
